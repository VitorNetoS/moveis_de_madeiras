...
<body>
	<div class='container box-mensagem-crud'>
		...
		if ($acao == 'incluir'):
			...
			$sql = 'INSERT INTO funcionario (nome, email, cpf, data_nacimento, telefone, endereco, foto)
							VALUES(:nome, :email, :cpf, :data_nacimento, :telefone, :endereco, :foto)';

			$stm = $conexao->prepare($sql);
			$stm->bindValue(':nome', $nome);
			$stm->bindValue(':email', $email);
			$stm->bindValue(':cpf', $cpf);
			$stm->bindValue(':data_nacimento', $data_ansi);
			...
			echo "<meta http-equiv=refresh content='3;URL=login_de_funcionario/cadastro.php'>";
		endif;
		...
		if ($acao == 'editar'):
			...
			$sql = 'UPDATE funcionario SET nome=:nome, email=:email, cpf=:cpf, data_nacimento=:data_nacimento, telefone=:telefone, endereco=:endereco, foto=:foto ';
			$sql .= 'WHERE id = :id';
			...
			echo "<meta http-equiv=refresh content='3;URL=lis_funcionario.php'>";
		endif;
		...
		if ($acao == 'excluir'):
			...
			$sql = "SELECT foto FROM funcionario WHERE id = :id AND foto <> 'padrao.jpg'";
			$stm = $conexao->prepare($sql);
			$stm->bindValue(':id', $id);
			$stm->execute();
			$funcionario = $stm->fetch(PDO::FETCH_OBJ);

			if (!empty($funcionario) && file_exists('fotos/'.$funcionario->foto)):
